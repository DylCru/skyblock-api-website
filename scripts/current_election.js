...
function getTotalVotes(data) {
    var totalVotes = 0;
    for (let i of data.current.candidates) {
        totalVotes += i.votes;
    }
    return totalVotes;
}
...
async function getElectionData() {
    ...
    document.getElementById("mayorYear").innerHTML += "Skyblock election: Year " + (data.current.year);
    document.getElementById("totalVotes").innerHTML += "Total Votes: " + totalVotes;

    console.log(data.current.candidates);
    can = data.current.candidates;
...
function makeMayorBox(i, canNum, totalVotes){
    ...
    var elec = `<div id="mayorBox" style="height:800px;  width:250px; margin-top: 0px; animation: moveIn 1.5s ease-out forwards ${canNum}s; background-image: linear-gradient(to top, ${boxColours[positions[4 - canNum]]} ${percent}%, rgba(255, 255, 255, 0.2) ${percent}%); " class="glassBox">
